...
export const ReporteAnulados = () => {
  ...
  const loadVentasAnuladas = async (fechaInicioParam = null, fechaFinParam = null) => {
    setLoading(true);
    try {
      const response = await apiClient.get('/ventas/anuladas');
      ...
      console.error('Error al cargar ventas anuladas:', error);
      setSnackbar({
        open: true,
        message: error.response?.data?.message || 'Error al cargar las ventas anuladas',
        severity: 'error'
      });
    } finally {
      setLoading(false);
  ...
  const handleSearch = () => {
    ...
    loadVentasAnuladas(fechaInicio || null, fechaFin || null);
  };
  ...
  const handleClearFilters = () => {
    ...
    loadVentasAnuladas(); // Recargar todas sin filtros
  };

  const handleViewPDF = (row) => {
    const ventaData = ventas.find(v => v.id_documento === row.id);
    const pdfDataFormatted = {
      nroDocumento: ventaData.nro_documento,
  ...
  const rows = ventas.map((venta, index) => ({
    ...
    total: venta.total,
    totalFormateado: formatTotal(venta.total),
    ventaData: venta
  }));
  ...
  return (
    <Box sx={{ width: '100%', overflowX: 'hidden', p: 3 }}>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Mostrando {rows.length} venta{rows.length !== 1 ? 's' : ''} anulada{rows.length !== 1 ? 's' : ''}
        {filtroActivo && ' (filtradas por fecha)'}
      </Typography>

      {/* Tabla de ventas anuladas */}
      {loading ? (
        <Preloader message="Cargando ventas anuladas..." fullScreen={false} />
      ) : (
        <DataTable 
          columns={columns} 
          rows={rows} 
